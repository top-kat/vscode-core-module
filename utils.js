...
function getObjFromCLIparams(argsWithNoValue) {
    let processArgs = JSON.parse(JSON.stringify(process.argv));
    if (processArgs[1] && processArgs[1].includes('/')) processArgs.splice(0, 2); // first 2 are path
    else processArgs.splice(0, 1); // edge case...
...
class dataValidationUtilErrorHandler extends Error {
    constructor(msg, status, extraInfo) {
        const { isProd } = configFn();
        super(msg);
        this.message = msg;
        this.name = `${status} ${msg}`;
        this.fromDataValidation = true; // will be catched by express error handler
        this.status = status;
        this.extraInfo = extraInfo;
        if (!isProd) {  // perf
            this.stack =
                dim('\n  ExtraInfos:' + JSON.stringify(extraInfo) + '\n') +
                // remove all lines containing node_module
                // AND the first line with error informations
                cleanStackTrace(this.stack) + '\n ';
        }
    }
}
 ...
 * @param {string} props `a.b.c.0.1` will test if myObject has props a that has prop b. Work wit arrays as well (like `arr.0`)
 */
function has(obj, props) {
    if (!isset(obj) || typeof obj !== 'object') return;
    let propsArr = props.replace(/\.?\[(\d+)\]/g, '.$1').split('.'); // replace a[3] => a.3;
    let objChain = obj;
...
function findByAddress(obj, addr) {
    ...
    return propsArr.reduce((objChain, prop) => {
...
function objForceWrite(obj, addr, item) {
    const chunks = addr.replace(/\.?\[\d\]/g, '.*').split('.');
    let lastItem = obj;
    chunks.forEach((chunk, i) => {
        if (i === chunks.length - 1) lastItem[chunk] = item;
        if (!isset(lastItem[chunk])) lastItem[chunk] = {};
...
function noDuplicateFilter(arr, comparisonFn = (a, b) => a === b) {
    if (Array.isArray(arr)) return arr.filter((a, i, arr) => arr.findIndex(b => comparisonFn(a, b)) === i);
    else return (e, i, arr) => arr.indexOf(e) === i; // legacy, used like arr.filter(noDuplicateFilter) only on katcha / linguapolis
}
...
function unflattenObject(data) {
    ...
    for (const p in data) {
        ...
        while ((m = regex.exec(p))) {
            if (prop === '*') cur = cur[cur.length] || (cur[cur.length] = (m[2] || m[1] === '*' ? [] : {}));
            else cur = cur[prop] || (cur[prop] = (m[2] || m[1] === '*' ? [] : {}));
            prop = (m[2] || m[1]).replace(/%/g, '.'); // allow propNames to contain dots
        }
        cur[prop] = data[p];
    }
...
function reassignForbidden(o) {
    return new Proxy(o, {
        setProperty: function (that, key, value) {
            if (key in that) throw new dataValidationUtilErrorHandler(`Cannot reassign the property ${key} of this object`, 500);
            return that[key] = value;
        },
        deleteProperty: function (_, key) {
            throw new dataValidationUtilErrorHandler(`Cannot delete the property ${key} of this object`, 500);
        }
    });
}
 ...
 * @return obj[addr] eventually processed by the callback
 */
function ensureIsArrayAndPush(obj, addr, valToPush, onlyUniqueValues = false) {
...
function errIfNotSet(objOfVarNamesWithValues, additionalMessage) { return errXXXIfNotSet(422, false, objOfVarNamesWithValues, additionalMessage); }
...
function checkCtxIntegrity(ctx) {
    if (!isset(ctx) || !isset(ctx.loggedUser)) throw new dataValidationUtilErrorHandler('ctxNotSet', 500);
}
...
function validatorReturnErrArray(...paramsToValidate) {
    ...
    for (const paramObj of paramsFormatted) {
        ...
        if (!isset(name)) {
            ...
            if (!isset(name)) throw new dataValidationUtilErrorHandler('noNameProvidedForDataValidator', 500, { origin: 'Generic validator', });
            value = paramObj[name];
        }
        ...
        if ((!isset(name) || !isset(value)) && optional) continue;
 ...
 */
function getDateAsInt(dateAllFormat = new Date(), errIfNotValid$ = false, withHoursAndMinutes$ = false) { // optional
    let dateInt;
    if (isDateIntOrStringValid(dateAllFormat)) {
        ...
        dateInt = '' + date.getUTCFullYear() + pad(date.getUTCMonth() + 1) + pad(date.getUTCDate()) + pad(date.getUTCHours()) + pad(date.getUTCMinutes());
    }
    isDateIntOrStringValid(dateInt, errIfNotValid$);
    return int(withHoursAndMinutes$ ? dateInt : dateInt.substr(0, 8));
}
...
function isDateIntOrStringValid(dateStringOrInt, outputAnError = false, length$ = false) {
...
function isDateIsoOrObjectValid(dateIsoOrObj, outputAnError = false) {
    ...
    let valid = dateObj instanceof Date && !isNaN(dateObj);
...
function dateArrayFormatted(dateAllFormat = getDateAsInt(), separator = false) {
    ...
    if (separator) return dateArr.join(separator);
    else return dateArr;
}
 ...
 */
function dateSlash(dateAllFormat) { return dateArrayFormatted(dateAllFormat, '/'); }
...
function getTimeAsInt(timeOrDateInt = getDateAsInt12()) {
    ...
    } else if (timeOrDateInt.length === 5 && timeOrDateInt.includes(':'))
 ...
 * @param {String} outputFormat dateInt, dateInt8, dateInt12, date, humanReadableTimestamp, int (dateInt8)
 */
function addWorkingDays(dateAllFormat = getDateAsInt(), numberOfWorkingDays = 1, outputFormat = 'int') {
    if (numberOfWorkingDays === 0) return getDateAs(date, dateAllFormat);
    let date = getDateAsObject(dateAllFormat);
    let realNumberOfDaysToAdd = numberOfWorkingDays;
    let day = date.getUTCDay();
    if (day === 6) {
        day = 0;
        dateAllFormat = addDays(dateAllFormat);
    }
    realNumberOfDaysToAdd += Math.trunc((day + numberOfWorkingDays - 1) / 5) * 2;
    return addDays(dateAllFormat, realNumberOfDaysToAdd, outputFormat);
}
/**
 * @param {String} outputFormat dateInt, dateInt8, dateInt12, date, humanReadableTimestamp, int (dateInt8)
 */
...
function getClosestExistingDateOfMonth(dateAllFormat = getDateAsInt()) {
    let [day, month, year] = dateArrayFormatted(dateAllFormat);
    const zeroBasedMonth = minMax(parseInt(month) - 1, 0, 11);
...
let isNodeEnv = (typeof process !== 'undefined') && (process.release.name === 'node');

const toRequire = isNodeEnv ? ['path', 'fs', 'glob', 'http'] : [];
const [path, fs, glob, http] = toRequire.map(mod => require(mod));

let loggerFileError = false; // prevent always throwing fileNotFoundError

const logger = {
    log(str, type = 'log', ctx) {
        const { preprocessLog, terminal, projectName } = configFn();
        ...
        // ^(\x1b.*?m)?\s*(\x1b.*?m)?$
        if (terminal.logRotate && isNodeEnv && !/^(\x1b.*?m)?\s*(\x1b.*?m)?$/.test(str)) this.processJson(str, type, ctx, terminal, projectName);
    ...
    raw: [],
    processJson(str, type, ctx, terminalConfig, projectName) {

        const { logRotateSeconds = 300, logRotateNbDaysToKeepLogs = 2, logMinifiedArrayFormat = false, logRotateAbsolutePath = '/logs', sendJsonLogOptions } = terminalConfig;

        let actualLog;
        let userId = has(ctx, 'loggedUser._id') ? ctx.loggedUser._id.toString() : undefined;
        if (logMinifiedArrayFormat) actualLog = [
            (new Date).toISOString(),
            type === 'error' ? 2 : ['warning', 'warn'].includes(type) ? 1 : ['log', 'info'].includes(type) ? 0 : type === 'appError' ? 3 : 4,
            str,
            userId,
        ];
        else actualLog = {
        ...
        if (typeof sendJsonLogOptions === 'object') {
            const body = JSON.stringify(actualLog);
            ...
            post_req.write(body);
            post_req.end();
        } else if (loggerFileError === false) {
            if (this.json.length === 0) setTimeout(() => {
                // WRITE LOG FILES TO SYSTEM
                // PERFS: 6.744ms MAX when log agression for file creation + deletion sync, 3ms average
                const toSave = this.json;
                this.json = [];

                const filePrefix = `${isset(projectName) ? projectName + '-' : ''}logs-`;
                const today = dateUtils.getDateAsInt12();
                const filePath = path.join(logRotateAbsolutePath, `${filePrefix}${today}.json`);

                fs.writeFile(filePath, JSON.stringify(toSave), err => {
                    if (err) {
                        console.error('DATA VALIDATION LOGGER ERROR');
                        console.error('Write file to ' + filePath + ' failed');
                        console.error('Error: ' + err);
                        loggerFileError = true;
                    }
                });

                // FLUSH OLDER LOG FILES
                const dateForOlder = new Date();
                dateForOlder.setUTCDate(dateForOlder.getUTCDate() - logRotateNbDaysToKeepLogs);
                const oldDate8 = dateUtils.getDateAsInt(dateForOlder);
                glob(path.join(logRotateAbsolutePath, `/${filePrefix}${oldDate8}*.json`), function (er, files) {
                    for (const file of files) fs.unlink(file, err => {
                        if (err) {
                            console.error('DATA VALIDATION LOGGER ERROR');
                            console.error('Write file to ' + filePath + ' failed');
                            console.error('Error: ' + err);
            ...
            }, logRotateSeconds * 1000);

            this.json.push(actualLog);
        }
    },
    json: [],
};
...
const C = {
    ...
    primary: str => C.rgb(...configFn().terminal.theme.primary) + str + C.reset,
    reset: '\x1b[0m',
    ...
    line(title = '', length = configFn().terminal.theme.pageWidth, clr = configFn().terminal.theme.primary, char = '=', paddingX = configFn().terminal.theme.paddingX, ctx) {
    ...
    log(...stringsCtxMayBeFirstParam) {
        let ctx;
        if (stringsCtxMayBeFirstParam[0] !== null && typeof stringsCtxMayBeFirstParam[0] === 'object' && stringsCtxMayBeFirstParam[0].isCtx) ctx = stringsCtxMayBeFirstParam.shift();
        stringsCtxMayBeFirstParam.forEach(str => this.logClr(str, undefined, undefined, ctx));
    },
    logClr(str, clr = configFn().terminal.theme.fontColor, paddingX = configFn().terminal.theme.paddingX, ctx) {
        if (!isset(str)) return;
        const padX = ' '.repeat(paddingX);
        str = padX + this.rgb(...clr) + str.toString().replace(/\n/g, '\n' + padX + this.rgb(...clr));
        logger.log(str + this.reset, 'log', ctx);
    },
    info(...str) {
        let ctx;
        if (str[0] !== null && typeof str[0] === 'object' && str[0].isCtx) ctx = str.shift();
        str.forEach((s, i) => {
            if (i === 0) this.logClr('ⓘ  ' + s, configFn().terminal.theme.primary, undefined, ctx);
            else this.log(ctx, this.dimStrSplit(s));
        });
        this.log(ctx, ' ');
    },
    success(...str) {
    ...
    gradientize(str = '', rgb1 = configFn().terminal.theme.shade1, rgb2 = configFn().terminal.theme.shade2, bgRgb = configFn().terminal.theme.bgClr, paddingY = configFn().terminal.theme.paddingY, paddingX = configFn().terminal.theme.paddingX) {
...
function logErrPrivate(type, color, ...errors) {
    ...
    let ctx;
    if (errors[0] !== null && typeof errors[0] === 'object' && errors[0].isCtx) ctx = errors.shift();
    ...
    const getStringFromErr = (err, i) => {
        if (!isset(err)) return '';
            ...
            const { str, stackTrace: stkTrc, ctx: extraInfoCtx } = stringifyInstanceOfError(err, type, color);
            if (stkTrc) stackTrace = stkTrc;
            if (isset(extraInfoCtx)) ctx = extraInfoCtx;
            return str;
        } else if (typeof err === 'object') {
            let msg = '';
            msg += removeCircularJSONstringify(err, 2).split('\n').map(val => C.dim(val)).join('\n') + '\n';
            const { str, stackTrace: stkTrc, ctx: extraInfoCtx } = stringifyExtraInfos(err.extraInfo || err, type, color);
            if (stkTrc) stackTrace = stkTrc;
            if (isset(extraInfoCtx)) ctx = extraInfoCtx;
            msg += str;
            return msg;
    ...
    if (errors.length && errors[0]) {
        ...
        logger.log(messages.join(''), type, ctx);
    }
}

function stringifyInstanceOfError(err, type = 'error', color = [255, 0, 0], level = 0) { // level = keep track of recursions
    ...
    if (isset(err.extraInfo)) {
        const { str: str2, stackTrace: stkTrc, ctx: extrCtx } = stringifyExtraInfos(err.extraInfo, type, color, level++);
        if (stkTrc) stackTrace = stkTrc;
        str += str2;
        if (isset(extrCtx)) ctx = extrCtx;
    }
    return { str, stackTrace, ctx };
}
...
function stringifyExtraInfos(extraInfoOriginal, type, color, level = 0) {
    ...
    if (extraInfoOriginal instanceof Error) { // case where error is passed directly to extraInfos
        ...
        const ctx = extraInfoOriginal.ctx;
        delete extraInfoOriginal.ctx;
        const extraInfo = { ...extraInfoOriginal };
        if (has(ctx, 'loggedUser._id')) extraInfo.userId = ctx.loggedUser._id.toString();
...
async function addCtxToErr(ctx, callback) {
    try {
        await callback();
    } catch (e) {
        if (!isset(e.extraInfo)) e.extraInfo = {};
        e.extraInfo.ctx = ctx;
        throw e;
    }
}
...
module.exports = {
    ...
    // ALIASES
    getDateAsArrayFormatted: dateArrayFormatted,
